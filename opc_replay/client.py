    ...
    try:
        ...
        if args.namespace_uri:
            ...
            try:
                ...
                print(f"\n✗ ERROR: Namespace URI '{args.namespace_uri}' not found on server", file=sys.stderr)
                print(f"Available namespaces:", file=sys.stderr)
                for i, ns in enumerate(namespaces):
            ...
            if target_ns >= len(namespaces):
                print(f"\n✗ ERROR: Namespace index {target_ns} does not exist on server", file=sys.stderr)
            ...
            if target_ns is None:
                print("\n✗ ERROR: No custom namespaces found (only standard ns=0 and ns=1)", file=sys.stderr)
